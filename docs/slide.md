...
## はじめまして、[nishitaku](https://x.com/nishitaku_dev)です
...
<!-- 
初めまして、nishitakuいいます。
本名はにしのたくろうといいます。フリーランスのエンジニアをやっています。
Angularが好きで、よく使っています。使ってる人少ないですが、ここ数年でかなり使いやすくなったので、ぜひ使ってみてほしいです。
-->
...
<!--
本セッションはSignals Deep Diveということで、15分間Signalについて語るわけですけれども、まず最初にこれだけ質問させてください。
signals知ってるよって人〜。その中で使ってる人〜。
...
<!-- 
まずはイントロから。
繰り返しになりますが、このセッションはSignals Deep Diveです。
単純にSignalsのAPIを呼び出して使うだけであれば、Deep Diveする必要はないです。Deep Diveの価値は、複雑なことをやろうとしたり、想定外の挙動にであったときに、なぜそうなるかを理解、説明できる点にあると思っています。
これからの15分間でsignalsがどういう仕組みで動いているのか、どうやって効率的に必要な箇所だけを更新しているのか、というsignalの内部実装を解説していきます。
一方で、signalsは各種フロントエンドFWも採用されていて、現代のリアクティブシステムの基盤になっているといえます。つまり、みなさんには、今日、このセッションを聞いていただいて、signalsの内部実装を少しでも理解してもらうことで、現代のリアクティブシステムの勘所を掴んでもらえればな、と思っております。
 -->
...
その後、signalsの大きな特徴として、依存関係の自動追跡、Push-Pullハイブリッド方式による遅延評価、メモ化の3つを順番に解説していきます。実際はもっとあるんですが、今回はこの3つを紹介します。もっと詳しく聞きたい人は、このあとask the speakerや懇親会でお話ししましょう。
...
フロントエンドの各種フレームワークで採用されています。自分もAngularで使っています。vueのrefとかsvelteのruneとかは若干見た目が違いますが、内部構造や考え方は同じです。
Signalsの基本概念として、State、Computed、Effectの3要素があります。このサンプルコードのように記述します。詳細は後述。
...
<!-- 
次に、signalsの2つ目の特徴として、Push-Pullハイブリッド方式による遅延評価について解説していきます。
-->
...
![bg cover](../images/welcome_to_angluar_v22.png)

<!-- 
と言いたいところですが、最後10秒だけください。
Angularは3日前にv22がリリースされました。
今日解説したsignalsを用いたSignal Formという画期的なAPIもstableになって、安心して本番利用できるようになって、めちゃくちゃいいアップデートです。
冒頭でも言いましたが、Angularはここ数年でものすごく使いやすくなってるので、ぜひ使ってみてほしいです。使い方知りたいとかであれば、いつでもご相談ください、という宣伝をもちまして、私の発表を終わりとさせていただきます。
-->
